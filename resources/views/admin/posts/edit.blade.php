...
@section('content')
    <div class="max-w-3xl mx-auto">
        <h1 class="text-3xl font-bold text-onBackground mb-8">Edit Post: {{ $post->title }}</h1>

        <form method="POST" action="{{ route('admin.posts.update', $post) }}" class="space-y-6 bg-surface p-6 rounded-lg shadow">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="6"
                          class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-primary bg-white text-onBackground">{{ old('content', $post->content) }}</textarea>
            </div>
